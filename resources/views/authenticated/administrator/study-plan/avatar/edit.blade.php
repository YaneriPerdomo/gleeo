    ...
    <main class="flex-grow-2 w-100 flex-and-direction-column flex-center-full flex-center-full-start">
        <article class="row main__article container-xl w-100">
            ...
            <div class="col-lg-10 col-12 main__content bg-white-border">
                ...
                <form action="{{ route('avatar.update', $data->slug) }}" class="form" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <x-input-text :item="[
                                ...
                                'form_input_value_default' => old('name_avatar', $data->name),
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="form__item">
                                <label for="avatar_path" class="form__label form__label--required">Imagen del Personaje:</label>
                                <div class="input-group">
                                    <span
                                        class="form__icon input-group-text @error('avatar_path') is-invalid--border @enderror"
                                        id="img-addon">
                                        <i class="bi bi-card-image"></i>
                                    </span>
                                    <input type="file" name="avatar_path" id="avatar_path"
                                        class="form-control @error('avatar_path') is-invalid @enderror"
                                        aria-label="Imagen del Negocio" style="display: none;" accept="image/*"
                                        data-img="{{ $data->slug ? 'true' : 'false' }}">
                                    <label for="avatar_path" class="btn btn-outline-secondary">Seleccionar
                                        Imagen</label>
                                    <span id="file-name-display" class="ms-3 text-muted">
                                        <input type="hidden" name="url" value="{{ $data->url }}">
                                        {{ $data->slug ? $data->url : 'Ningún archivo seleccionado' }}
                                    </span>
                                </div>
                                ...
                                <script>
                                    document.addEventListener('DOMContentLoaded', function() {
                                        const fileInput = document.getElementById('avatar_path');
